...
class ValueTransformerTests: XCTestCase {
    ...
    func testReversibleValueTransformer() {
        ...
        ValueTransformer.setValueTransformer(withName: "testReversibleTransformer",
                                             transform: toString,
                                             reverseTransform: toInt)
    ...
    func testDictionaryTransformer() {
        ...
        ValueTransformer.setDictionaryTransformer(withName: "testDictionaryTransformer",
                                                  transform: preprocessJSONDictionary)
        ...
        guard let transformed = transformedDictionary?["transformed"] as? Bool else {
    ...
    func testEntityMapper() {
        ...
        XCTAssertEqual("ConcreteA", transformer.transformedValue(["type": "A"]) as? String,
                       "should call the transform function")
        XCTAssertEqual("ConcreteB", transformer.transformedValue(["type": "B"]) as? String,
                       "should call the transform function")
        XCTAssertNil(transformer.transformedValue(nil), "should handle nil values")
    }
}
